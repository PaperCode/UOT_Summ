...
def compute_two_sinkhorn_loss(rouge_matrices,
    ...
    c_m = (1 - rouge_matrices) * x_y_mask + (1.0 - x_y_mask) * 2.0
    # c_m = rouge_matrices * x_y_mask + (1.0 - x_y_mask) * 20.0
    ...
    return ot_cost_positive, src_marginal, ot_cost_negative
...
def sinkhorn_batched(cost_matrix,
                     mu, nu,
                     is_balanced=False,
                     epsilon=1.0, tau=1.0, max_num_iter=3000):
    # print("mu", mu)
    # print("nu", nu)
    ...
    for i in range(max_num_iter):
        ...
        if err_1 < 2e-2:  # todo The termination threshold should be small enough!!!
            break
    # print("iteration", i)
