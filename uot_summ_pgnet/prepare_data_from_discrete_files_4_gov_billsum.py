...
def load_files(d_path, sub_dir_name,  configs):  # 4 discrete files
    f_path = d_path + sub_dir_name
    # print("f_path", f_path)

    all_files = listdir(f_path)
    # print("all_files", all_files)

    file_tuples = []
    for one_file in all_files:
        with open(join(f_path, one_file)) as input_json_file:
            data = json.load(input_json_file)

            if len(data['sections']) > 0 and len(data['abstract']) > 0:
                x = read_article_sections(data['sections'], configs)
                y = read_abstract_sents(data['abstract'], configs)

                # print("len(x[0])", len(x[0]))
                # print("len(y[0])", len(y[0]))
                # if len(x) > 0 and len(y) > 0 and len(rouge_matrix) > 0:
                # if len(x[0]) > 0 and len(y[0]) > 0:
                # print("one_file", one_file)
                # print("x", x)
                # print("y", y)
                file_tuples.append((x, y, one_file))
                # print("=======================")

    return file_tuples


def read_article_sections(article_sections_list, cfg):
    # todo handle too short sentence -> no
    article_sections_list = article_sections_list[0:cfg.MAX_SRC_SECTIONS_NUMS]
    tokenized_sections = []
    for one_section in article_sections_list:
        ...
        for one_sent in one_section:
            ...
            if len(words_one_sent) == 0:
                continue

            if len(words_one_section) + len(words_one_sent) + 1 <= cfg.MAX_LEN_ONE_SECTION_SRC:  # 1 for eos
...
def read_abstract_sents(abstract_sents_list, cfg):
    # todo handle too short sentence ??
    abstract_sents_list = abstract_sents_list[0: cfg.MAX_ABST_SENTENCES_NUMS]

    tokenized_sents = []
    for one_sent in abstract_sents_list:
        one_sent = one_sent.strip("\n").lower()
        words = one_sent.split()
        words = words[0: cfg.MAX_LEN_ONE_SENTENCE_ABST-1]

        if len(words) == 0:  # todo: change abstract_sents_list
            continue
    ...
    return tokenized_sents, abstract_sents_list


'''
def read_article_sents(article_sents_list, cfg):
    # todo handle too short sentence ??
    article_sents_list = article_sents_list[0:cfg.MAX_SRC_SEN_NUMS]
    tokenized_sents = []
    for one_sent in article_sents_list:
        one_sent = one_sent.strip("\n").lower()
        words = one_sent.split()
        words = words[0:cfg.MAX_LEN_ONE_SEN_SRC]
    ...
    return tokenized_sents, article_sents_list
    
def read_rouge_matrix(rouge_matrix, cfg):
    # todo handle too short sentence ??

    # print("len(rouge_matrix)", len(rouge_matrix))
    # print("len(rouge_matrix)", len(rouge_matrix[0]))
    rouge_matrix = rouge_matrix[0:cfg.MAX_SRC_SEN_NUMS]
    trimmed_rouge_matrix = []
    for one_line in rouge_matrix:
        trimmed_rouge_matrix.append(one_line[0:cfg.MAX_ABST_SEN_NUMS])

    return trimmed_rouge_matrix
...
def to_dict_4_discrete_files(xyfns, dic):
    # dict should not consider test set!!!!!
    for xyfn in xyfns:
        articles, summs, _ = xyfn
        # print("len(articles)", len(articles))
        # print("articles", articles)
        # print("len(summs)", len(summs))
        # print("summs", summs)

        for sen_summ in summs[0]:
            for w in sen_summ:
                # print("w", w)
                if w in dic:
                    dic[w] += 1
...
def prepare_deepmind(d_path):
    ...
    print("trainset...")
    train_xy_fn_list = load_files(d_path, "train", configs)
    # train_xy_list = load_lines(d_path, "train", configs)

    print("dump train...")
    pickle.dump(train_xy_fn_list, open(TRAINING_PATH + "train.pkl", "wb"), protocol=pickle.HIGHEST_PROTOCOL)
    ...
    dic_from_data = {}
    # dic_list = []
    # all_dic, dic_list = load_dict(d_path, "vocab", all_dic, dic_list)  # not suitable for gov
    # dic_from_data = to_dict(train_xyr_list, dic_from_data)
    dic_from_data = to_dict_4_discrete_files(train_xy_fn_list, dic_from_data)
    sorted_dic_from_data = collections.OrderedDict(
        sorted(dic_from_data.items(), key=operator.itemgetter(1), reverse=True))
    print("sorted_dic_from_data", sorted_dic_from_data)

    dic_list = list(sorted_dic_from_data.keys())
    print("dic_list", dic_list)

    # for w, tf in dic_from_data.items():
    #     if w not in all_dic:
    #         all_dic[w] = tf
    ...
    print("dump dict...")
    # useless: dic_from_data hfw w2w
    pickle.dump([dic_from_data, dic, hfw, w2i, i2w, w2w], open(TRAINING_PATH + "dic.pkl", "wb"),
                protocol=pickle.HIGHEST_PROTOCOL)
    
    print("testset...")
    test_xy_fn_list = load_files(d_path, "test", configs)
    # test_xy_list = load_lines(d_path, "test.txt", configs)

    print("validset...")
    valid_xy_fn_list = load_files(d_path, "val", configs)
    # valid_xy_list = load_lines(d_path, "val.txt", configs)

    print("#train = ", len(train_xy_fn_list))
    print("#test = ", len(test_xy_fn_list))
    print("#validate = ", len(valid_xy_fn_list))
        
    print("#all_dic = ", len(dic_from_data), ", #dic = ", len(dic), ", #hfw = ", len(hfw))

    print("dump test...")
    pickle.dump(test_xy_fn_list, open(TESTING_PATH + "test.pkl", "wb"), protocol=pickle.HIGHEST_PROTOCOL)
    shuffle(test_xy_fn_list)
    pickle.dump(test_xy_fn_list[0:2000], open(TESTING_PATH + "pj2000.pkl", "wb"), protocol=pickle.HIGHEST_PROTOCOL)

    print("dump validate...")
    pickle.dump(valid_xy_fn_list, open(VALIDATE_PATH + "valid.pkl", "wb"), protocol=pickle.HIGHEST_PROTOCOL)
    pickle.dump(valid_xy_fn_list[0:1000], open(VALIDATE_PATH + "pj1000.pkl", "wb"), protocol=pickle.HIGHEST_PROTOCOL)
...
if __name__ == "__main__":
    ...
    # raw_path = "/gds/xshen/projdata11/researchPJ/ot_abs/toy_set_discrete_files/"
    # raw_path = "/gds/xshen/projdata11/researchPJ/ot_abs/processed_arxiv/"
    raw_path = "/gds/xshen/projdata17/researchPJ/processed_gov/"
    # raw_path = "/gds/xshen/projdata11/researchPJ/new_uot/CNNDM_R1R2RL_F/"
...
'''
def read_sents(article_sents_list, cfg):  # read_cont(
    tokenized_sents = []
    for one_sent in article_sents_list:
        one_sent = one_sent.strip("\n").lower()
        words = one_sent.split()
        ...
        if num_words < cfg.MAX_LEN_ONE_SEN:
            tokenized_sents.append(words)
        else:
            tokenized_sents.append(words[0:cfg.MAX_LEN_ONE_SEN])

    return tokenized_sents, article_sents_list

# def read_abst(f_cont, cfg):
#     return
# def read_rouge(f_cont, cfg):
#     return
'''
