...
class BeamSearch(Registrable):
    ...
    def __init__(
        ...
        # print("final_sequence_scorer", final_sequence_scorer)
        self.final_sequence_scorer = final_sequence_scorer or SequenceLogProbabilityScorer()  # later one
        ...
        print("self.min_steps", self.min_steps)
    ...
    @staticmethod
    def are_enough_sents_generated(predictions, summ_sents_nums_to_generate, eos_index):
        # predictions shape: [(batch_size, beam_size)]
        # print("predictions", predictions)
        # print("summ_sents_nums_to_generate", summ_sents_nums_to_generate)
        # print("eos_index", eos_index)
        assert predictions[0].size(0) == summ_sents_nums_to_generate.size(0)
        batch_size = predictions[0].size(0)
        beam_size = predictions[0].size(1)
        summ_sents_nums_to_generate_expand_in_beam = summ_sents_nums_to_generate.unsqueeze(1) \
            .expand(batch_size, beam_size)
        # print("summ_sents_nums_to_generate_expand_in_beam", summ_sents_nums_to_generate_expand_in_beam.size())
        # print("summ_sents_nums_to_generate_expand_in_beam", summ_sents_nums_to_generate_expand_in_beam)

        # shape: (timestep * batch_size * beam_size,)
        stacked_predictions = torch.stack(predictions, dim=0)
        # print("stacked_predictions", stacked_predictions.size())

        nums_of_generated_eos = torch.sum((stacked_predictions == eos_index).float(), dim=0, keepdim=False)
        # print("nums_of_generated_eos", nums_of_generated_eos.size())
        # print("nums_of_generated_eos", nums_of_generated_eos)
        satisfied = torch.ge(nums_of_generated_eos, summ_sents_nums_to_generate_expand_in_beam)
        # print("satisfied", satisfied.size())
        # print("satisfied", satisfied)

        return satisfied

    @staticmethod
    def are_enough_sents_generated_unique_consecutive(predictions, summ_sents_nums_to_generate, eos_index):
        # predictions shape: [(batch_size, beam_size)]
        # print("predictions", predictions)
        # print("summ_sents_nums_to_generate", summ_sents_nums_to_generate)
        # print("eos_index", eos_index)
        ...
        stacked_predictions = torch.stack(predictions, dim=0)
        # print("stacked_predictions", stacked_predictions.size())
        # print("stacked_predictions", stacked_predictions)

        eos_num_one_batch = []
        for i in range(batch_size):
            eos_num_one_beam = []
            for j in range(beam_size):
                # print("stacked_predictions[:, i, j]", stacked_predictions[:, i, j])
                prediction_ij = torch.unique_consecutive(stacked_predictions[:, i, j])
                # print("prediction_ij", prediction_ij)
                num_of_eos = torch.sum((prediction_ij == eos_index).float(), dim=0, keepdim=False)
                # print("num_of_eos", num_of_eos)
                eos_num_one_beam.append(num_of_eos.item())
            eos_num_one_batch.append(eos_num_one_beam)

        # print("eos_num_one_batch 1", eos_num_one_batch)
        eos_num_one_batch = torch.Tensor(eos_num_one_batch).to(stacked_predictions.get_device())
        # print("eos_num_one_batch 2", eos_num_one_batch)

        # nums_of_generated_eos = torch.sum((stacked_predictions == eos_index).float(), dim=0, keepdim=False)
        # print("nums_of_generated_eos", nums_of_generated_eos.size())
        # print("nums_of_generated_eos", nums_of_generated_eos)
        satisfied = torch.ge(eos_num_one_batch, summ_sents_nums_to_generate_expand_in_beam)
        # print("satisfied", satisfied.size())
        # print("satisfied", satisfied)

        # if satisfied.all().item():
        #     print("eos_num_one_batch", eos_num_one_batch)
        #     print("summ_sents_nums_to_generate_expand_in_beam", summ_sents_nums_to_generate_expand_in_beam)
        #     print("stacked_predictions", torch.transpose(torch.transpose(stacked_predictions, 0, 1), 1, 2))

        return satisfied

    @staticmethod
    def are_enough_sents_generated_reconstructed_ver(all_predictions, summ_sents_nums_to_generate, eos_index):
        # predictions shape: [(batch_size, beam_size)]
        # all_predictions shape: (batch_size * beam_size * timestep ,)
        # print("all_predictions", all_predictions)
        # print("all_predictions size", all_predictions.size())
        # print("summ_sents_nums_to_generate", summ_sents_nums_to_generate)
        # print("eos_index", eos_index)
        assert all_predictions.size(0) == summ_sents_nums_to_generate.size(0)
        ...
        for i in range(batch_size):
            ...
            for j in range(beam_size):
                # print("stacked_predictions[:, i, j]", stacked_predictions[:, i, j])
                prediction_ij = torch.unique_consecutive(all_predictions[i, j, :])
                # print("prediction_ij", prediction_ij)
                num_of_eos = torch.sum((prediction_ij == eos_index).float(), dim=0, keepdim=False)
                # print("num_of_eos", num_of_eos)
                eos_num_one_beam.append(num_of_eos.item())
        ...
        # print("eos_num_one_batch 1", eos_num_one_batch)
        eos_num_one_batch = torch.Tensor(eos_num_one_batch).to(all_predictions.get_device())
        # print("eos_num_one_batch 2", eos_num_one_batch)
    ...
    def search(
            ...
            summ_sents_nums_to_generate: torch.Tensor,
        ...
        # print("step_signature.parameters", step_signature.parameters)
    ...
    def _search(
        ...
        # print("start_class_log_probabilities", start_class_log_probabilities.size())
        ...
        # todo min_value_of_dtype: Returns the minimum value of a given PyTorch data type. Does not allow torch.bool.
        # Prevent selecting the end symbol if there is any min_steps constraint
        if self.min_steps >= 1:  # todo not used
        ...
        # print("predictions start", predictions)
        ...
        # print("log_probs_after_end 1", log_probs_after_end.size())
        # print("log_probs_after_end 1", log_probs_after_end)
        # print("log_probs_after_end.sum() 1", log_probs_after_end.sum())
        log_probs_after_end[:, self._end_index] = 0.0
        # print("log_probs_after_end 2", log_probs_after_end)
        ...
        for timestep in range(self.max_steps - 1):
            ...
            # print("last_predictions mimi", last_predictions.size())
            # print("last_predictions mimi", last_predictions)

            # todo moved from loop out *********************************************
            # print("predictions mumu", predictions)
            ...
            #     break
            # todo new for ot:
            # enough_sents_generated = self.are_enough_sents_generated(
            #     predictions, summ_sents_nums_to_generate, eos_index=self._end_index)
            # enough_sents_generated = self.are_enough_sents_generated_unique_consecutive(
            #     predictions, summ_sents_nums_to_generate, eos_index=self._end_index)
            enough_sents_generated = self.are_enough_sents_generated_reconstructed_ver(
                all_predictions, summ_sents_nums_to_generate, eos_index=self._end_index)
            if enough_sents_generated.all().item():
                # print("enough_sents_generated", enough_sents_generated)
                break
            ...
            if timestep + 2 <= self.min_steps:  # todo not used
            ...
            #     batch_size * self.beam_size, num_classes
            # )
            # print("last_predictions_expanded", last_predictions_expanded.size())
            # print("last_predictions_expanded", last_predictions_expanded)
            ...
            # )
            # todo modified for ot
            enough_sents_generated_expanded = \
                (enough_sents_generated.reshape(batch_size * self.beam_size).unsqueeze(-1)). \
                    expand(batch_size * self.beam_size, num_classes)
            # print("enough_sents_generated_expanded", enough_sents_generated_expanded.size())
            # print("enough_sents_generated_expanded", enough_sents_generated_expanded)
            ...
            # print("cleaned_log_probabilities mimi", cleaned_log_probabilities)

            # shape (both): (batch_size * beam_size, per_node_beam_size)
            # top_log_probabilities, predicted_classes, sampler_state = self.sampler.sample_nodes(
            #     class_log_probabilities, self.per_node_beam_size, sampler_state
            # )
            # todo original
            top_log_probabilities, predicted_classes, sampler_state = self.sampler.sample_nodes(
                cleaned_log_probabilities, self.per_node_beam_size, sampler_state
            )
            # print("top_log_probabilities kiki", top_log_probabilities)
            ...
            # print("summed_top_log_probabilities", summed_top_log_probabilities.size())
            # print("summed_top_log_probabilities", summed_top_log_probabilities)

            # shape: (batch_size, beam_size * per_node_beam_size)
            reshaped_summed = summed_top_log_probabilities.reshape(
                batch_size, self.beam_size * self.per_node_beam_size
            )
            # print("reshaped_summed", reshaped_summed.size())
            ...
            # print("timestep: ", timestep, " predictions: ", predictions)

            # shape: (batch_size, beam_size)
            last_log_probabilities = restricted_beam_log_probs
            # print("last_log_probabilities", last_log_probabilities)
        ...
        # shape: (batch_size, beam_size)
        # todo self._end_index is not used, final_scores == last_log_probabilities
        # print("last_log_probabilities", last_log_probabilities)
...
'''
        # self._end_of_sent = end_of_sent
        # end_of_sent: int,

    @staticmethod
    def count_end_of_sent(predictions, backpointers):
    ...
    def _reconstruct_sequences(predictions, backpointers):
        # Reconstruct the sequences.
        # shape: [(batch_size, beam_size, 1)]
        # print("predictions _reconstruct_sequences", predictions)
        # print("backpointers _reconstruct_sequences", backpointers)
        reconstructed_predictions = [predictions[-1].unsqueeze(2)]

        if not backpointers:
            # print("not backpointers")
            return reconstructed_predictions
        ...
        for timestep in range(len(predictions) - 2, 0, -1):
            # shape: (batch_size, beam_size, 1)
            # print("predictions[timestep]", predictions[timestep])
            # print("cur_backpointers", cur_backpointers)
            cur_preds = predictions[timestep].gather(1, cur_backpointers).unsqueeze(2)
            # print("cur_backpointers", cur_backpointers)
            # print("********************************")
